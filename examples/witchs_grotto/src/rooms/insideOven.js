...
import { initFlue, setInsideOven } from "./flue";

let insideOven, hottingUp;

export const getInsideOven = () => insideOven;

export const initInsideOven = () => {
  insideOven = new Room(
    "inside oven",
  ...
  openDoor.onFailure = "It seems to be locked somehow. Who on Earth needs a lockable oven, anyway?";

  const unlockDoor = lockedOvenDoor.getVerb("unlock");
  unlockDoor.test = false;
  unlockDoor.onFailure = "You can't see the locking mechanism. Must be on the other side.";

  lockedOvenDoor.addVerb(
    new Verb("kick", false, null, "You succeed in little more than hurting yourself. The door doesn't budge.", [
      "boot",
      "shove",
      "punch",
      "hit"
  ...
  const flueItem = new Item("flue", () => {
    ...
    if (fireBurning) {
      description += " You can see tongues of flame licking up the pipe from below.";
    } else {
...
export const getHottingUp = () => hottingUp;

export const initHottingUp = () => {
  hottingUp = new Schedule.Builder()
    .withCondition(() => getRoom() === getInsideOven())
