...
import { initDiningRoom } from "./diningRoom";
import { initPantry } from "./pantry";
import { initEntranceHall } from "./entranceHall";
import { initInsideOven } from "./insideOven";
...
An archway leads West, a rickety door barely hangs on its hinges to the South and a sturdier looking door leads East.`
  ...
  const diningRoom = initDiningRoom();
  const pantry = initPantry();
  const entranceHall = initEntranceHall();
  const insideOven = initInsideOven();
  ...
  const cabinet = new Item(
    "cabinet",
    "It's made of grey, dirty oak and comes up to your shoulders. It looks as though it's seen better days, with nails visible between the various panels where it's gradually coming apart. There's a single door taking up most of the front."
  );
  cabinet.capacity = 8;
  ...
  cabinetDoor.verbs["close"].onSuccess.insertAction(() => (cabinet.itemsVisibleFromSelf = false));
